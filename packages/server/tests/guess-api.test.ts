  ...
  describe('guess API logic', ({ test }) => {
    test('should process valid guess request successfully', async () => {
      const request: GuessRequest = {
        ...
        puzzleId: '§2025-11-11',
        guess: ['M', 'O', 'G', 'B', 'O'], // correct answer
      }
      ...
      expect(answer).toEqual(['M', 'O', 'G', 'B', 'O'])
    ...
    test('should handle case insensitive guesses', async () => {
      const request: GuessRequest = {
        deviceId: 'test-device',
        puzzleId: '§2025-11-11',
        guess: ['m', 'o', 'g', 'b', 'o'] as unknown as ValidColor[], // lowercase
      }
    ...
    test('should provide correct feedback for partial matches', async () => {
      const request: GuessRequest = {
        deviceId: 'test-device',
        puzzleId: '§2025-11-11',
        guess: ['O', 'B', 'M', 'G', 'Y'], // O present, B present, M present, G present, Y absent
      }
